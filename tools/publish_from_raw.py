...
- Selection enforces MAX_TIME_DIFF_MINUTES to avoid wrong publish when RAW not uploaded yet.
  If closest candidate diff > limit -> exit code 4 (NO_RAW_YET).
...
Verification:
- After upload, poll IA metadata until the file appears (with sleep).
"""
...
def sanitize_filename(s: str) -> str:
    """
    IA için güvenli ASCII dosya adı üretir.
    - Türkçe/Almanca karakterleri normalize eder.
    - Sadece [A-Za-z0-9 ._-] bırakır.
    """
    s = s.strip()
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(3)#!/usr/bin/env python3
"""
tools/publish_from_raw.py
