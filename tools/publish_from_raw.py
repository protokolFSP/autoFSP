...
def parse_line(line: str) -> ParsedLine:
    ...
    if re.fullmatch(r"\d{1,2}", maybe_hour) and maybe_date.count(".") == 2:
        ...
        if hour < 0 or hour > 23:
            raise PublishError(f"Hour out of range (0-23): {hour}")
        date_str = maybe_date
        core = parts[:-2]
    else:
        date_str = parts[-1]
    ...
    if date_str.count(".") != 2:
        raise PublishError(f"Invalid date: '{date_str}' (expected DD.MM.YYYY)")
    ...
    return ParsedLine(title=title, speaker=speaker, assistant=assistant, lesson_date=lesson_dt, hour=hour)
...
def build_candidates(raw_identifier: str, session: requests.Session) -> List[Candidate]:
    if ZoneInfo is None:
        raise PublishError("zoneinfo not available in this runtime (unexpected on GH Actions).")
...
def select_candidate(
    ...
    if used >= len(same_day_sorted):
        ...
        raise PublishError(f"Hour missing and no remaining candidates for {key}. times=[{times}] used={used}")
...
def main() -> int:
    ...
    print(
        ...
        f"Berlin={chosen.berlin_dt:%Y-%m-%d %H:%M}"
    )
    ...
    print(json.dumps(
        {
            ...
            "hour_used_for_selection": p.hour,
